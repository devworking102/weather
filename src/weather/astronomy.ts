import { CHI, HOANG_DAO_BY_MONTH } from './constants'

/* ── Lunar calendar (Âm lịch) ────────────────────────────────────────────
 * Ho Ngoc Duc algorithm (public-domain, widely used for VN lunar calendars).
 * Converts Gregorian → Vietnamese lunar date (UTC+7).
 */
...
/** 12 zodiacal 2-hour blocks. Block 0 = Tý (23:00–01:00), 1 = Sửu (01:00–03:00)... */
export interface ZodiacHour { index: number; name: string; start: number; end: number; good: boolean }
...
/** Simplified moon phase from days since known new moon. */
